...
# --- Configuration ---
DB_FILE = os.getenv(
    "HISTORY_DB_FILE", os.getenv("SCHEDULE_DB_FILE", "playsched.db")
)  # SQLite database file name for history and synced playlists
...
def update_history_db(sp, conn):
    ...
    try:
        ...
        conn.commit()
        print(
            f"History update complete. Added: {added_count} new entries. Skipped (already present): {skipped_count} entries."
        )
        ...
        while True:
            try:
                ...
                track_results = sp.playlist_items(
                    playlist_id, limit=track_limit, offset=track_offset, fields=fields_param
                )
                ...
                print(
                    f"  Spotify API error fetching tracks for playlist '{playlist_name}': {e.msg}. Skipping tracks for this playlist."
                )
                spotify_playlist_track_items = []
                break
            except Exception as e:
                print(
                    f"  Unexpected error fetching tracks for playlist '{playlist_name}': {e}. Skipping tracks for this playlist."
                )
                spotify_playlist_track_items = []
                break
        print(
            f"  Retrieved {len(spotify_playlist_track_items)} valid tracks from Spotify API for playlist '{playlist_name}'."
        )
...
def get_spotify_client():
    ...
    try:
        ...
        print(
            " 4. If a browser window opened, the auth flow may have failed (check Redirect URI in Spotify Dashboard matches .env EXACTLY, including https://)."
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    action_group.add_argument(
        "--list-playlists", action="store_true", help="List your playlists (direct from Spotify API)."
    )
    ...
    action_group.add_argument(
        "--sync-playlists", action="store_true", help="Sync all user playlists and tracks to local DB."
    )
    ...
    is_action_flag_present = any(
        [args.list_devices, args.list_playlists, args.update_history, args.recent_playlists, args.sync_playlists]
    )
    ...
    try:
        ...
        if is_export_action:
            ...
            if args.list_devices:
                ...
                update_history_db(sp, conn)
            elif args.recent_playlists:
                show_recent_playlists(sp, conn, market_code)
            elif args.sync_playlists:
                sync_all_playlists_and_tracks(sp, conn)
            ...
            if device_id and playlist_uri:
                try:
                    ...
                    if "Restriction violated" in str(e.msg):
                        print("Hint: Check device status/Premium.")
                except Exception as e:
