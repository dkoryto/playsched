...
)  # Use a default ONLY for dev if not set
...
def calculate_next_play_time_utc(schedule, now_utc):
    ...
    for i in range(8):  # Check today + next 7 days
        ...
        try:
            ...
            logger.error(
                f"[Calc {schedule_id_log}]: Error combining date {check_date_utc} / time {start_time_local_obj}: {combine_e}. Skipping day {i}."
            )
            continue

        # *** Explicitly Localize the naive datetime using the schedule's timezone ***
        try:
            localized_potential_dt = schedule_tz.localize(
                naive_potential_dt, is_dst=None
            )  # is_dst=None handles ambiguity/non-existent times
        except (pytz.AmbiguousTimeError, pytz.NonExistentTimeError) as loc_e:
            logger.warning(
                f"[Calc {schedule_id_log}]: Timezone localization issue for {naive_potential_dt} in {tz_str}: {loc_e}. Skipping potential time."
            )
            # Optionally, try is_dst=True/False or add/subtract an hour if needed, but skipping is safer
            continue
        except Exception as e:
            logger.error(
                f"[Calc {schedule_id_log}]: Error localizing time {naive_potential_dt} in {tz_str}: {e}. Skipping day {i}."
            )
            continue
        ...
        logger.debug(
            ...
            f"PotentialLocal={localized_potential_dt.isoformat()}, "
            f"PotentialUTC={potential_dt_utc.isoformat()}, LocalWeekday={potential_weekday_local}"
        )
        ...
        if potential_dt_utc > now_utc:
            logger.info(
                f"[Calc {schedule_id_log}]: Found next future UTC time: {potential_dt_utc.isoformat()}. Returning."
            )
...
def api_play_schedule_now(schedule_id):
    ...
    if success:
        ...
        app.logger.info(
            f"Attempting to set shuffle state to {shuffle_enabled} for manual play (Schedule {schedule_id})..."
        )
        try:
            ...
            app.logger.info(
                f"Shuffle state set to {shuffle_enabled} successfully for manual play (Schedule {schedule_id}) after {sleep_duration}s delay."
            )
            ...
            app.logger.warning(
                f"Could not set shuffle state to {shuffle_enabled} for manual play (Schedule {schedule_id}): {shuffle_e}"
            )
        except Exception as general_shuffle_e:
            app.logger.error(
                f"Unexpected error setting shuffle state for manual play (Schedule {schedule_id}): {general_shuffle_e}",
                exc_info=True,
            )
...
@app.route("/api/schedules/<int:schedule_id>/stop_now", methods=["POST"])
def api_stop_schedule_now(schedule_id):
    ...
    device_id = schedule_info["target_device_id"]
    app.logger.info(f"Manual Stop Now for Schedule {schedule_id}: Device={device_id}")
    ...
    try:
        ...
        # Pause on the active device (not passing device_id explicitly, as pause_playback
        # works on the currently active device)
        sp.pause_playback()
...
@app.route("/api/current_playback", methods=["GET"])
