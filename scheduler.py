...
def perform_spotify_action(sp, schedule, logger):
    ...
    logger.info(
        f"Performing action '{action}' for schedule {schedule_id} (User: {user_id}), Shuffle: {shuffle_enabled}"
    )
    ...
    try:
        if action == "start_playback":
            ...
            try:
                ...
                logger.info(
                    f"Shuffle state set to {shuffle_enabled} successfully for schedule {schedule_id} after {sleep_duration}s delay."
                )
                ...
                logger.warning(
                    f"Could not set shuffle state to {shuffle_enabled} for schedule {schedule_id} (Device: {device_id}): {shuffle_e}"
                )
            except Exception as general_shuffle_e:
                logger.error(
                    f"Unexpected error setting shuffle state for schedule {schedule_id}: {general_shuffle_e}",
                    exc_info=True,
                )
        ...
        if start_time_str == current_time_str:
            ...
            if is_play_once:
                ...
                try:
                    ...
                    logger.warning(
                        f"[Start Check {schedule_id}]: Error processing days_of_week '{days_of_week_str}': {day_e}. Skipping."
                    )
                    continue
            ...
            if is_due_today:
                ...
                if last_triggered_iso:
                    try:
                        ...
                        logger.warning(
                            f"[Start Check {schedule_id}]: Error comparing last_triggered_utc: {dt_e}. Proceeding cautiously."
                        )
        ...
        if stop_time_str and device_id and tz_str:
            try:
                ...
                if current_time_str == stop_time_str:
                    logger.info(
                        f"[Stop Check {schedule_id}]: Stop time matches current time ({current_time_str} in {tz_str}). Checking playback state..."
                    )
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        logger.info(
                                            ...
                                            f"scheduled ({playlist_uri_to_match})."
                                        )
                                else:
                                    logger.info(
                                        f"[Stop Check {schedule_id}]: Skipping pause - playback is active, but not on target device ({device_id})."
                                    )
                            else:
                                logger.info(
                                    f"[Stop Check {schedule_id}]: Skipping pause - Spotify reports no active playback."
                                )

                            # --- Send Pause Command if Checks Pass ---
                            if should_pause:
                                logger.info(
                                    f"Attempting to PAUSE playback for schedule {schedule_id} on device {device_id}..."
                                )
                                try:
                                    ...
                                    logger.warning(
                                        f"Could not send PAUSE command for schedule {schedule_id} (Device: {device_id}): {pause_e}"
                                    )
                                except Exception as general_pause_e:
                                    logger.error(
                                        f"Unexpected error sending PAUSE command for schedule {schedule_id}: {general_pause_e}",
                                        exc_info=True,
                                    )

                        except SpotifyException as state_e:
                            logger.warning(
                                f"[Stop Check {schedule_id}]: Could not get current playback state: {state_e}"
                            )
                        except Exception as general_state_e:
                            logger.error(
                                f"[Stop Check {schedule_id}]: Unexpected error checking playback state: {general_state_e}",
                                exc_info=True,
                            )
                    else:
                        logger.warning(
                            f"Scheduler: Skipping STOP action for schedule {schedule_id} - could not get client."
                        )
